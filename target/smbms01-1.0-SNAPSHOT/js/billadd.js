...
$(function(){
	...
	$.ajax({
		...
		success:function(data){//data：返回数据（json对象）
			if(data != null){
				...
				var options = "<option value=\"0\" disabled>请选择</option>";
		...
		error:function(data){//当访问时候，404，500 等非200的错误状态码
			validateTip(providerId.next(),{"color":"red"},imgNo+" error",false);
		}
	});
	/*
	 * 验证
	...
	productDesc.on("focus",function(){
		...
		if(productDesc.val() != null && productDesc.val() != ""){
			...
			validateTip(productDesc.next(),{"color":"red"},imgNo+" 类别不能为空，请重新输入",false);
		}
	...
	providerId.on("focus",function(){
		...
		if(providerId.val() != null && providerId.val() != "" && providerId.val() != 0){
			...
			validateTip(providerId.next(),{"color":"red"},imgNo+"供应商不能为空，请重新输入",false);
		}
		
	});
	
	productCount.on("focus",function(){
		validateTip(productCount.next(),{"color":"#666666"},"* 请输入大于0的数，小数点后保留两位",false);
	}).on("keyup",function(){
	...
	totalPrice.on("focus",function(){
		validateTip(totalPrice.next(),{"color":"#666666"},"* 请输入大于0的正整数，小数点后保留两位",false);
	}).on("keyup",function(){
		this.value = priceReg(this.value);
	}).on("blur",function(){
		this.value = priceReg(this.value);
	...
	backBtn.on("click",function(){
		...
		// 	history.back(-1);
		// }
		window.location.href="/jsp/bill?method=query";
	});
});