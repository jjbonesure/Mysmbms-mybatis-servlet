...
<div class="right">
        ...
        <div class="providerAdd">
          <form id="billForm" name="billForm" method="post" action="${pageContext.request.contextPath }/jsp/bill">
				...
				<input type="hidden" name="id" value="${sessionScope.bill.id }">
                <!--div的class 为error是验证错误，ok是验证成功-->
                <c:if test="${info=='true'}">
                    <span style="color: green;font-size: large">修改成功！</span>
                </c:if>
                <c:if test="${info=='false'}">
                    <span style="color: red;font-size: large">修改失败！</span>
                </c:if>
              <div class="">
              ...
              <div>
                  <label for="productDesc">商品类别：</label>
                  <input type="text" name="productDesc" id="productDesc" value="${bill.produceDesc }">
                  <font color="red"></font>
              </div>
                <div>
